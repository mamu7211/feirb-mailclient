...
1. **Read the issue** from GitHub:
   ...
   ```
   The issue body contains the full feature spec including deliverables and acceptance criteria.

2. **Check for dependent issues** mentioned in the issue body:
...
3. **Check for uncommitted changes** before switching branches:
   ```bash
   git status
   ```
...
5. **Create a feature branch from main** using the issue number and a slug derived from the issue title:
   ```bash
...
7. **Implement the feature** following project conventions from CLAUDE.md:
   ...
   - **For any API endpoint work:** use `/implement-bruno-tests` to create Bruno API tests
   - **For any UI work with routes:** use `/implement-playwright-tests` to create Playwright E2E tests
